...
export class MarketDataService {
  ...
  private generateForexOpportunities(): TradingOpportunity[] {
    ...
    // Major forex pairs with current market dynamics
    const forexPairs = [
      { 
        symbol: "EUR/USD", 
        name: "Euro / US Dollar", 
    ...
    forexPairs.forEach((pair, index) => {
      ...
      const microLotSize = 1000; // Micro lot (1/100th of standard)
      const pipValue = pair.symbol === "USD/JPY" ? 0.01 : 0.0001;
      const targetPips = Math.round(50 + (pair.volatility * 100));
      const stopPips = Math.round(25 + (pair.volatility * 50));
      
      const action = pair.bias === "bullish" ? "BUY" : "SELL";
      const direction = action === "BUY" ? "higher" : "lower";
      
      // Calculate profit/loss in USD
      const potentialProfit = (targetPips * pipValue * microLotSize);
      const potentialLoss = (stopPips * pipValue * microLotSize);
      const marginRequired = microLotSize * pair.currentPrice * 0.02; // 2% margin
      
      opportunities.push({
        id: `forex-${pair.symbol.replace('/', '').toLowerCase()}`,
        name: `${pair.name}`,
        type: `Micro Lot • ${pair.symbol}`,
        entryPrice: `${pair.currentPrice.toFixed(4)}`,
        risk: `-$${potentialLoss.toFixed(2)}`,
        potentialGain: `+$${potentialProfit.toFixed(2)}`,
        netProfit: `+$${(potentialProfit - potentialLoss).toFixed(2)}`,
        confidence: Math.round(75 + (index * 3)),
        action: action as "BUY" | "SELL",
        market: "forex",
        rationale: this.generateForexRationale(pair, action, targetPips, stopPips),
        isMicro: true,
        lotSize: "Micro Lot (1,000 units)",
        pipValue: `$${(pipValue * microLotSize).toFixed(2)} per pip`,
        spread: `${(pair.spread * 10000).toFixed(1)} pips`,
        marginRequired: `$${marginRequired.toFixed(2)}`,
        leverage: "50:1",
        targetPips: `${targetPips} pips`,
        stopLoss: `${stopPips} pips`
      });

      // Add nano forex for ultra-small positions (only for major pairs)
      if (index < 3) {
        const nanoLotSize = 100; // 1/10th of micro lot
        const nanoPotentialProfit = (targetPips * pipValue * nanoLotSize);
        const nanoPotentialLoss = (stopPips * pipValue * nanoLotSize);
        const nanoMarginRequired = nanoLotSize * pair.currentPrice * 0.02;
        
        opportunities.push({
          id: `nano-forex-${pair.symbol.replace('/', '').toLowerCase()}`,
          name: `Nano ${pair.name}`,
          type: `Nano Lot • ${pair.symbol}`,
          entryPrice: `${pair.currentPrice.toFixed(4)}`,
          risk: `-$${nanoPotentialLoss.toFixed(2)}`,
          potentialGain: `+$${nanoPotentialProfit.toFixed(2)}`,
          netProfit: `+$${(nanoPotentialProfit - nanoPotentialLoss).toFixed(2)}`,
          confidence: Math.round(82 + (index * 2)),
          action: action as "BUY" | "SELL",
          market: "forex",
          rationale: this.generateForexRationale(pair, action, targetPips, stopPips, true),
          isMicro: true,
          lotSize: "Nano Lot (100 units)",
          pipValue: `$${(pipValue * nanoLotSize).toFixed(3)} per pip`,
          spread: `${(pair.spread * 10000).toFixed(1)} pips`,
          marginRequired: `$${nanoMarginRequired.toFixed(2)}`,
          leverage: "50:1",
          targetPips: `${targetPips} pips`,
          stopLoss: `${stopPips} pips`
        });
  ...
  private generateForexRationale(pair: any, action: string, targetPips: number, stopPips: number, isNano: boolean = false): string {
    const direction = action === "BUY" ? "upward" : "downward";
    const trend = action === "BUY" ? "bullish" : "bearish";
    const sizeText = isNano ? "Nano lot provides ultra-small position sizing for precise risk management." : "Micro lot offers reduced position size with professional execution.";
    
    const fundamentalReasons = {
      "EUR/USD": "ECB monetary policy divergence from Fed creating opportunities. European data showing resilience.",
      "GBP/USD": "Bank of England maintaining hawkish stance. UK inflation data supporting currency strength.",
      "USD/JPY": "BoJ intervention concerns at current levels. Technical resistance forming near 150.00.",
      "AUD/USD": "RBA policy outlook supporting AUD. Commodity prices providing tailwind to resource currencies.",
      "USD/CHF": "Swiss National Bank policy shifts affecting franc dynamics. Safe-haven flows influencing direction."
    };

    const reason = fundamentalReasons[pair.symbol as keyof typeof fundamentalReasons] || "Central bank policy divergence creating trading opportunities.";
    
    return `${reason} Technical analysis supports ${direction} momentum with ${trend} bias. Target: ${targetPips} pips profit | Risk: ${stopPips} pips maximum loss. ${sizeText} Professional spread execution with institutional liquidity. Position sized for optimal risk-reward ratio.`;
  }
}
