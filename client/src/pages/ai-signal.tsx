...
function SignalModal({opp,onClose,onTrade,trading}:{opp:TradingOpportunity;onClose:()=>void;onTrade:()=>void;trading:boolean}) {
  ...
  return(
    <div className="fixed inset-0 z-50 flex items-end justify-center" style={{background:"rgba(0,0,0,0.7)"}}>
      <div className="w-full max-w-md rounded-t-3xl p-6 animate-fade-in" style={{background:"#1a2332",border:"1px solid #243044",maxHeight:"90vh",overflowY:"auto"}}>
...
export default function AISignalPage() {
  ...
  const tradeMutation=useMutation({
    mutationFn:async(opp:TradingOpportunity)=>{
      const res=await fetch("/api/trades/execute",{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${token}`},
  ...
  return(
    <div className="page-container min-h-screen" style={{background:"#0d1117"}}>
      <div className="max-w-md mx-auto px-4 pt-6">
        ...
        {/* Cards */}
        {isLoading?(
          <div className="flex flex-col gap-3">{Array(5).fill(0).map((_,i)=><div key={i} className="skeleton rounded-2xl" style={{height:160}}/>)}</div>
        ):filtered.length===0?(
