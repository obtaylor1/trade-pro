  ...
  return (
    <div className="page-container lg:pb-8 min-h-screen" style={{ background: "#0d1117" }}>
      <div className="px-4 lg:px-8 pt-6 max-w-none">
        ...
        {/* Trade cards — 3 cols on desktop */}
        {isLoading ? (
          <div className="grid lg:grid-cols-3 gap-4">
            {Array(6).fill(0).map((_, i) => <div key={i} className="skeleton rounded-2xl" style={{ height: 280 }} />)}
          </div>
        ) : (
          <div className="grid lg:grid-cols-3 gap-4">
            {(opps ?? []).map(opp => (
