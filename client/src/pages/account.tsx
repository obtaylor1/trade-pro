  ...
  return (
    <div className="page-container lg:pb-8 min-h-screen" style={{ background: "#0d1117" }}>
      <div className="px-4 lg:px-8 pt-6 max-w-none">
        ...
        {/* Profile + Stats side by side on desktop */}
                  ...
                  return (
                    <div key={trade.id} className="rounded-xl p-3" style={{ background: "#1a2332", border: "1px solid #243044" }}>
                      ...
                      <div className="flex items-center justify-between">
                        <div className="text-xs" style={{ color: "#64748b" }}>{parseFloat(trade.units).toFixed(4)} units @ ${parseFloat(trade.entryPrice).toFixed(2)}</div>
                        {isOpen && (
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr style={{ borderBottom: "1px solid #243044" }}>
                      {["Ticker", "Market", "Action", "Entry Price", "Units", "Invested", "P&L", "Status", "Date", ""].map(h => (
                  ...
                  <tbody>
                    {allTrades.map((trade: any, idx: number) => {
                      ...
                      const isOpen = trade.status === "OPEN";
                      return (
                        <tr key={trade.id} style={{ borderBottom: idx < allTrades.length - 1 ? "1px solid #1a2332" : "none", background: idx % 2 === 0 ? "transparent" : "rgba(255,255,255,0.01)" }}>
                          <td className="px-4 py-3 font-bold">{trade.ticker}</td>
                          <td className="px-4 py-3">
                            <span className="text-xs px-2 py-0.5 rounded-full capitalize" style={{ background: "rgba(59,130,246,0.15)", color: "#60a5fa" }}>{trade.market}</span>
                          </td>
                          <td className="px-4 py-3">
                            <span className="text-xs font-bold" style={{ color: trade.action === "BUY" ? "#22c55e" : "#ef4444" }}>{trade.action}</span>
                          </td>
                          ...
                          <td className="px-4 py-3 font-mono text-xs">${invested.toFixed(2)}</td>
                          <td className="px-4 py-3 font-bold" style={{ color: isOpen ? "#94a3b8" : pnl >= 0 ? "#22c55e" : "#ef4444" }}>
                            {isOpen ? "—" : `${pnl >= 0 ? "+" : ""}$${pnl.toFixed(2)}`}
                          </td>
                          <td className="px-4 py-3">
                            <span className="text-xs px-2 py-0.5 rounded-full font-bold" style={{ background: isOpen ? "rgba(34,197,94,0.15)" : "rgba(100,116,139,0.15)", color: isOpen ? "#22c55e" : "#94a3b8" }}>{trade.status}</span>
                          </td>
